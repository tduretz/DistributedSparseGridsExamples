...
MC_N = 100_000 # Number of Monte Carlo snapshots
...
q_res_x = Matrix{Float64}(undef,8,nt_max)
q_res_y = Matrix{Float64}(undef,8,nt_max)
exp_val_res = zeros(Float64, 2, nt_max)

for i = 1:MC_N
    exp_val_res .+= MC_res[i]
end
exp_val_res ./= MC_N
...
@showprogress Threads.@threads for val_i = 1:nt_max
    ...
    for i in 1:MC_N
        quant_vals_x[i] = MC_res[i][1,val_i]
        quant_vals_y[i] = MC_res[i][2,val_i]
    end
...
function plot_quantile!(ax, x1,y1,x2,y2,color)
    idx  = max(findfirst(x->isapprox(x,0.0,atol=1e-6), y1), findfirst(x->isapprox(x,0.0,atol=1e-6), y2))
    pts = vcat(
        Point2f.(x1[1:idx], y1[1:idx]),
        reverse(Point2f.(x2[1:idx], y2[1:idx]))
    )
...
plot_quantile!(ax, q_res_x[1,:],q_res_y[1,:],q_res_x[end,:],q_res_y[end,:],RGBA(250/255, 160/255, 160/255,0.4))
plot_quantile!(ax, q_res_x[2,:],q_res_y[2,:],q_res_x[end-1,:],q_res_y[end-1,:],RGBA(250/255, 160/255, 160/255,0.4))
plot_quantile!(ax, q_res_x[3,:],q_res_y[3,:],q_res_x[end-2,:],q_res_y[end-2,:],RGBA(255/255, 234/255, 0/255,0.4))
plot_quantile!(ax, q_res_x[4,:],q_res_y[4,:],q_res_x[end-3,:],q_res_y[end-3,:],RGBA(80/255, 200/255, 120/255,0.4))
