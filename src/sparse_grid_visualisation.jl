...
function GLMakie.surface!(ax, npts = 20, postfun=x->x; kwargs...) where {CT,CP<:AbstractCollocationPoint{2,CT},HCP<:AbstractHierarchicalCollocationPoint{2,CP},SG<:AbstractHierarchicalSparseGrid{2,HCP}}
    xs = LinRange(-1.0, 1.0, npts)
    ys = LinRange(-1.0, 1.0, npts)
    zs = [begin;
            postfun([x, y])
